...
import { Book, BorrowRecord, LibraryContextType } from '../types'
...
export const LibraryProvider = ({ children }: { children: ReactNode }) => {
  ...
  // Admin: book add করা
  const addBook = useCallback((bookData: Omit<Book, 'id' | 'rating' | 'reviewCount' | 'borrowCount' | 'createdAt'>) => {
    const newBook: Book = {
      ...bookData,
      id:           String(Date.now()),
      rating:       0,
  ...
  // Admin: book edit করা
  const updateBook = useCallback((bookId: string, updatedData: Partial<Book>) => {
    setBooks((prev) =>
  ...
  const approveBorrow = useCallback((borrowId: string) => {
    setBorrows((prev) =>
  ...
  const rejectBorrow = useCallback((borrowId: string) => {
    const borrow = borrows.find((b) => b.id === borrowId)
    if (!borrow) return
