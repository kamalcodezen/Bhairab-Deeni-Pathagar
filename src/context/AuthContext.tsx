...
export const AuthProvider = ({ children }: { children: ReactNode }) => {
  // वर्तमान লগইন করা user এর state
  ...
  useEffect(() => {
    try {
      ...
      if (savedUser && savedSession) {
        ...
        const sessionExpiry: number = JSON.parse(savedSession)
  ...
  // ভবিষ্যতে এখানে backend API call করা যাবে
  const register = useCallback(async ({ name, email, password, phone }: { name: string; email: string; password: string; phone?: string }) => {
