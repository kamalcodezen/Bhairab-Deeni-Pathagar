...
import { Upload, Book as BookIcon, Save, ArrowLeft } from 'lucide-react'
...
const AddBook: React.FC = () => {
  ...
  const { register, handleSubmit, formState: { errors, isSubmitting } } = useForm<AddBookFormInput>()
  ...
  const onSubmit: SubmitHandler<AddBookFormInput> = async (data) => {
    try {
      const newBook = {
        ...
        categorySlug: data.category.toLowerCase().replace(/[^a-z0-9]/g, '-'),
        availableCopies: parseInt(data.copies, 10),
        totalCopies: parseInt(data.copies, 10),
        pages: parseInt(data.pages || '0', 10) || 0,
